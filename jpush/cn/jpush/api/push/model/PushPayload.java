...
public class PushPayload implements PushModel {
	...
	private PushPayload(Platform platform, Audience audience, Notification notification, Message message,
			Options options) {
	...
	public static class Builder {
		...
		/**
		 * 设置推送的设备类型(Android /IOS /Windows Phone) 必填
		 * @param platform
		 * @return
		 */
		...
		/**
		 * 通知内容体。是被推送到客户端的内容。与 message 一起二者必须有其一，可以二者并存 可选
		 * @param notification
		 * @return
		 */
		...
		/**
		 * 消息内容体。是被推送到客户端的内容。与 notification 一起二者必须有其一，可以二者并存 可选
		 * @param message
		 * @return
		 */
