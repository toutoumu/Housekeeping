...
public class JsonUtils {
	...
	/**
	 * 获取json数据中的元素，如"file":"file.file" 
	 * @param str 原始字符串
	 * @param arrt 属性
	 * @return 属性对应的值 如"file":"file.file"返回file.file
	 */
	public static String getAttr(String str, String arrt) {
		JsonParser jsonParser = new JsonParser();
		JsonElement element = jsonParser.parse(str);
		element = element.getAsJsonObject().get(arrt);
	 ...
	 * @return 属性对应的值 如"file":"file.file"返回file.file
	 */
	public static JsonElement getObject(String str, String arrt) {
		JsonParser jsonParser = new JsonParser();
		JsonElement element = jsonParser.parse(str);
		element = element.getAsJsonObject().get(arrt);
		return element;
	 ...
	 * <ul>
	 * <li>该方法会转换标有 {@literal @Expose} 注解的字段；</li>
	 ...
	 * <li>该方法不会转换 {@code null} 值字段；</li>
	 * <li>该方法会转换所有未标注或已标注 {@literal @Since} 的字段；</li>
	 * <li>该方法转换时使用默认的 日期/时间 格式化模式 - {@code yyyy-MM-dd HH:mm:ss S}；</li>
	 * </ul>
	 * 
	 * @param target 要转换成 {@code JSON} 的目标对象。
	 * @param targetType 目标对象的类型。
